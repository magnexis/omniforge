...
if (-not $docker) {
  Write-Error "docker is required to run the container-backed Haskell worker"
  exit 1
}
...
& docker compose --profile functional run --rm -T omniforge-functional runghc workers/haskell/worker.hs
exit $LASTEXITCODE
